...
def _find_executable_lines(f: TextIO) -> Iterator[str]:
    ...
    while True:
        ...
        buffer += char
        if buffer[-2] == "\n" and buffer[-1] not in " \t\n\r":
            yield buffer[:-2].strip()
            buffer = buffer[-1]


def _write_local_scope(local_scope: dict[str, Any]) -> int:
    ...
    Returns:
        Length of the message printed.
    """
    width = shutil.get_terminal_size()[0]
    if local_scope:
        print(f"\n{Ansi.bright_blue}Locals:{Ansi.reset}")
    ...
    with path.open(mode="r", encoding="UTF-8") as f:
        ...
        for expression in _find_executable_lines(f):
            print(
                ...
                + f"\n{Ansi.bright_magenta}...{Ansi.reset} ".join(
                    expression.split("\n")
                )
            )
            ...
            _write_local_scope(local_scope)
            ...
            print(
                (
                    Ansi.cursor_up * (len(local_scope) + 3)
